...
def search(fileNum, texts, rep2_locate, data_added):
    ...
    for ii in range(beg2, end2):
        ...
        for sent in sents:
            ...
            res = p.findall(sent)
            if len(res) > 0:
                # print(fileNum, "***", sent.strip())
                out = str(fileNum) + " *** " + sent.strip() + "\n"
                out2.append(out)
...
f222 = open("C:/Users/li/Desktop/year.txt", "w")
...
for file in files:
    ...
    out2 = search(fileNum, texts, rep2_locate, data_added)
    for out in out2:
        f222.write(out)
